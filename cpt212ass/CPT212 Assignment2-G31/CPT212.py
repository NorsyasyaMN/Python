...
'''CPT212 ASSIGNMENT 2 BY Muaadh Esmail Saif Abdulkhaleq 149686, NG GUO FOONG 147453, CHIN KAH SING 146655'''
...
class Graph:
    ...
    def CheckStronglyConnected(self):
        ...
        for n in range(0, 4):
            if not visited[n]:
                self.printGraph()
                print(colored("NOT STRONGLY CONNECTED... GENERATING A NEW GRAPH", 'red'))
                return False
    ...
    """-----------------Function 4: Compute minimum spanning tree--------------"""
        ...
        while choice:
            if choice == 1:
                if g.StronglyConnected():
                    g.printGraph()
                    print(colored("THE GRAPH ABOVE IS STRONGLY CONNECTED", 'green'))
                    AWAIT_TRIGGER()
                    cls_screen()
                break
            elif choice == 2:
